...
import kz.runtime.backfor_mega.entityjson.CoinJson;
...
//        nameCoin.add("magic");
        ...
        for (var str : nameCoin){
            ...
            for (Map.Entry<String, Double> set : arr.entrySet()) {
                ...
                } else if (set.getKey().equals("usd_24h_change")) {
                    crypto.setChange(set.getValue());
                }else if(set.getKey().equals("last_updated_at")){
                    long seconds = Math.round (set.getValue()); // пример значения количества секунд
